...
<?php
        ...
        $showParPage = 3;
        $showTotalesReq = $bdd->query('SELECT notes FROM addgame WHERE notes ="5"');
        ...
        <?php
        $show = $bdd->query('SELECT * FROM addgame WHERE notes = "5" ORDER BY id DESC LIMIT ' . $depart . ',' . $showParPage);
  ...
  for ($i = 1; $i <= $pagesTotales; $i++) {
    if ($i == $pageCourante) {
      ...
      echo '<li class="page-item "><a class="page-link " <a href="bestnote.php?page=' . $i . '">' . $i . '</a> ';
    }
  }
  ?>
  </table>
