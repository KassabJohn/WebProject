...
<?php
        ...
        $showParPage = 3;
        $showTotalesReq = $bdd->query('SELECT genre FROM addgame WHERE genre = "Automobile"');
        ...
        <?php
        $show = $bdd->query('SELECT * FROM addgame WHERE genre = "Automobile" ORDER BY id DESC LIMIT ' . $depart . ',' . $showParPage);
  ...
  for ($i = 1; $i <= $pagesTotales; $i++) {
    if ($i == $pageCourante) {
      ...
      echo '<li class="page-item "><a class="page-link " <a href="auto.php?page=' . $i . '">' . $i . '</a> ';
    }
  }
  ?>
  </table>
